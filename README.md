...
- Для использования makefile из корня репозитория необходимо выполнить команду `make`. По умолчанию делается билд и пуш всех образов, указанных в makefile. Если необходимо запустить только определенный билд - то команду необходимо указать с параметром `-e`, а после параметра указать действие `build`, пример - `make IMAGE_PATH=./monitoring/prometheus build`.
